...
export default function TrialModal() {
  ...
  useEffect(() => {
    ...
    const onValueDelivered = () => setValueAt((v) => v + 1);
    arm();
    window.addEventListener("bys:checkin-value", onValueDelivered);
    return () => {
      window.removeEventListener("bys:checkin-value", onValueDelivered);
      if (timerRef.current) clearTimeout(timerRef.current);
      timerRef.current = null;
    };

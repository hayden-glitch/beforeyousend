...
import { IconAction, IconArrowDown, IconArrowUp, IconBook, IconClose, IconDownload, IconGavel, IconHistory, IconLog, IconOrganizer, IconPlus, IconReview, IconTimeline } from "~/components/icons";
...
function CommunicationLog({ onReviewReply, child, tier, catchUp, onCatchUpDone }: { onReviewReply?: (message: string) => void; child?: ChildInfo | null; tier?: string; catchUp?: any; onCatchUpDone?: () => void }){
 ...
 {!selected&&!editing&&<><div className="mt-4 flex flex-wrap gap-1.5"><button onClick={()=>setFilter("all")} className={`chip min-h-11 px-3.5 py-2 text-sm ${filter==="all"?"border-forest bg-forest text-cream":""}`}>All ({logs.length})</button>{TOPICS.map(t=><button key={t} onClick={()=>setFilter(t)} className={`chip min-h-11 px-3.5 py-2 text-sm ${filter===t?"border-forest bg-forest text-cream":""}`}>{t}</button>)}</div>{shown.length===0?<p className="card mt-4 p-5 text-base text-stone">Nothing logged yet — when an exchange happens, add it here.</p>:<div className="card mt-4 divide-y divide-line overflow-hidden">{shown.map(x=><button key={x.id} onClick={()=>setSelected(x)} className="block w-full px-5 py-4 text-left transition hover:bg-cream-deep/40"><span className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-1.5 text-sm font-medium text-forest">{x.direction==="sent"?<><IconArrowUp className="h-4 w-4"/>Sent by me</>:<><IconArrowDown className="h-4 w-4"/>Received</>}</span><span className="shrink-0 text-sm text-taupe">{new Date(x.date).toLocaleDateString()}</span></span><p className="mt-1.5 line-clamp-2 text-base text-ink">{x.message}</p><span className="mt-1 block text-sm text-stone">{x.topic}{toneSuffix(x.tone)}{x.child&&child?.id===x.child?` · ${child?.name}`:null}</span></button>)}</div>}</>}

...
function CommunicationLog({ onReviewReply, child, tier, catchUp, onCatchUpDone }: { onReviewReply?: (message: string) => void; child?: ChildInfo | null; tier?: string; catchUp?: any; onCatchUpDone?: () => void }){
 const [logs,setLogs]=useState<any[]>([]), [filter,setFilter]=useState("all"), [selected,setSelected]=useState<any>(null), [editing,setEditing]=useState(false), [form,setForm]=useState<any>({message:"",direction:"sent",date:localDT(),topic:"other",notes:"",tone:"",child:""}), [notice,setNotice]=useState("");
...
function EventTimeline({ tier, organizerAccess, onGoLog, onGoOrganizer, onReviewReply }: { tier: string; organizerAccess?: boolean; onGoLog: () => void; onGoOrganizer: () => void; onReviewReply?: (message: string) => void }) {
  ...
  {total>0&&!selected&&!editing&&<div className="mt-5 rounded-3xl border border-line bg-card p-5 shadow-card">
    ...
    <div className={`mt-3 grid ${suite ? "grid-cols-4" : "grid-cols-3"} gap-2`} role="group" aria-label="Filter your record">
      {KIND_TILES.filter(([k]) => suite || k !== "documents").map(([k, label, Icon]) => (
        <button key={k} type="button" onClick={() => { setKind(k as any); track("timeline_ribbon_filter", { kind: k }); }}
          className={`chip min-h-11 justify-center whitespace-nowrap ${kind === k ? "border-forest bg-forest text-cream" : ""}`}>
          <Icon className="h-4 w-4" />{label}<span className="ml-1">{counts[k as keyof typeof counts]}</span>
        </button>
      ))}
    </div>
  </div>}
  {!selected&&!editing&&<>{showIncidentChrome&&<div className="mt-5 flex gap-2 overflow-x-auto pb-2"><button onClick={()=>setFilter("all")} className={`chip whitespace-nowrap ${filter==="all"?"border-forest bg-forest text-cream":""}`}>All ({items.length})</button>{TIMELINE_CATEGORIES.map(([k,l])=><button key={k} onClick={()=>{if(kind!=="incidents")setKind("incidents");setFilter(k)}} className={`chip whitespace-nowrap ${filter===k?"border-forest bg-forest text-cream":""}`}>{l}</button>)}</div>}
  {shown.length===0?<div className="mt-4 rounded-3xl border border-line bg-card p-6 text-base text-stone">{kind==="messages"?(<>No messages in your record yet. <button type="button" onClick={onGoLog} className="font-semibold text-forest underline underline-offset-4">Open the Communication Log →</button></>):kind==="incidents"?"No incidents here yet.":kind==="documents"?"No documents here yet.":"Nothing recorded yet. Add pickup problems, school incidents, medical visits, court dates — anything worth remembering."}</div>:<div className="mt-4 space-y-6">{groups.map(g=><div key={g.key}><h3 className="font-display text-xl font-semibold text-forest">{monthLabel(g.key)}</h3><div className="mt-2 space-y-3">{g.rows.map(m=>m.kind==="incident"?(<button key={m.id} onClick={()=>setSelected(m.src)} className="w-full rounded-3xl border border-line bg-card p-5 text-left shadow-card"><div className="flex items-center justify-between gap-3"><span className="rounded-full border border-line bg-cream-deep px-3 py-1 text-sm font-medium text-forest">{m.sub}</span><span className="flex shrink-0 items-center gap-2"><span className="rounded-full border border-line bg-cream-deep px-3 py-1 text-sm font-medium text-forest">Incident</span><span className="text-base text-stone">{fmtDate(m.dateKey)}</span></span></div><p className="mt-2 font-semibold text-forest">{m.title}</p>{m.body&&<p className="mt-1 line-clamp-2 text-base text-ink">{m.body}</p>}</button>):(<button key={m.id} onClick={()=>{setSelected(m);track("timeline_item_view",{kind:m.kind})}} className="w-full rounded-3xl border border-line bg-card p-5 text-left shadow-card"><div className="flex items-center justify-between gap-3"><span className="rounded-full border border-line bg-cream-deep px-3 py-1 text-sm font-medium text-forest">{m.kind==="message"?"Message":"Document"}</span><span className="shrink-0 text-base text-stone">{fmtDate(m.dateKey)}</span></div><p className="mt-2 line-clamp-2 font-semibold text-forest">{m.title}</p><p className="mt-1 text-sm text-stone">{m.sub}</p>{m.kind==="document"&&m.body&&<p className="mt-1 line-clamp-1 text-base text-ink">{m.body}</p>}</button>))}</div></div>)}</div>}</>}
  ...
  {editing&&<div className="mt-5 rounded-3xl border border-line bg-card p-6"><button onClick={reset} className="min-h-11 text-base text-forest underline">← Cancel</button><label className="field-label mt-5">Date</label><input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} className="min-h-12 w-full rounded-2xl border border-line bg-cream px-4"/><label className="field-label mt-4">Title</label><input type="text" value={form.title} onChange={e=>setForm({...form,title:e.target.value})} maxLength={200} placeholder="What happened?" className="min-h-12 w-full rounded-2xl border border-line bg-cream px-4"/><label className="field-label mt-4">Category</label><div className="flex flex-wrap gap-2">{TIMELINE_CATEGORIES.map(([k,l])=><button key={k} type="button" onClick={()=>setForm({...form,category:k})} className={`chip ${form.category===k?"border-forest bg-forest text-cream":""}`}>{l}</button>)}</div><label className="field-label mt-4">Details <span className="font-normal text-stone">(optional, only visible to you)</span></label><textarea rows={4} value={form.details} onChange={e=>setForm({...form,details:e.target.value})} placeholder="A few factual notes: who, what, when, where…" className="w-full rounded-2xl border border-line bg-cream p-4 text-base"/><button onClick={save} disabled={!form.title.trim()} className="btn-primary mt-5 w-full">{editing&&selected?"Save changes":"Add to timeline"}</button></div>}
  {notice&&<p className="mt-4 rounded-2xl bg-cream-deep px-4 py-3 text-base text-stone" role="status">{notice}</p>}</section>;
}
function Dashboard(){
 ...
 return <div className="min-h-dvh pb-24"><header className="sticky top-0 z-20 min-h-16 border-b border-line/70 bg-cream/95 backdrop-blur"><div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-5 py-2"><a href="/" className="font-display text-xl font-semibold text-forest sm:text-2xl">Before You Send<span className="text-forest-soft">.</span></a><UserMenu user={user} tier={tier} context="dashboard" /></div></header>

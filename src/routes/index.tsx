...
export const Route = createFileRoute("/")({
  head: () => ({
    ...
    // Landing hero A/B split: assign bys_hero_variant (a|b, 50/50, stable per
    // visitor) BEFORE first paint so no variant flashes. Sets data-hero-variant
    // on <html>; app.css shows exactly one H1 (default A). Idempotent. The
    // script also mirrors the CSS onto aria-hidden at DOMContentLoaded so the
    // hidden variant is out of the a11y tree even for DOM-level audits
    // (backlog 2ee6e58e). Visual + cookie behavior unchanged.
    scripts: [
      { tag: "script", children: `(function(){try{var c=document.cookie.match(/(?:^|;\\s*)bys_hero_variant=([^;]+)/);var v=(c&&(c[1]==="a"||c[1]==="b"))?c[1]:(Math.random()<0.5?"a":"b");if(!c)document.cookie="bys_hero_variant="+v+"; Max-Age=31536000; Path=/; SameSite=Lax";document.documentElement.setAttribute("data-hero-variant",v);function ah(){var a=document.querySelector("h1.hero-variant-a"),b=document.querySelector("h1.hero-variant-b");if(a)a.setAttribute("aria-hidden",v==="b"?"true":"false");if(b)b.setAttribute("aria-hidden",v==="a"?"true":"false")}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",ah);else ah()}catch(e){}})();` },
...
function heroVariant(): "a" | "b" {
  if (typeof document === "undefined") return "a";
  return document.documentElement.getAttribute("data-hero-variant") === "b" ? "b" : "a";
}

...
function Login(){
  ...
  const head = purchaseCtx
    ? { h: "Continue to checkout", s: mode==="signin" ? "Sign in to finish your purchase — we'll bring you right back." : "Create your free account to finish your purchase — we'll bring you right back. No card needed." }
    : reviewCtx
      ? { h: mode==="signin" ? "Sign in" : "Keep this on your record", s: mode==="signin" ? "Welcome back." : "Your account saves your reviews and builds your record — free, no card." }
      : { h: mode==="signin" ? "Sign in" : "Create your free account", s: mode==="signin" ? "Welcome back." : "No card needed." };
  return <main id="main" tabIndex={-1} className="mx-auto flex min-h-dvh max-w-lg flex-col justify-center px-5 py-12"><a href="/" className="flex min-h-11 items-center gap-2 text-2xl font-semibold text-forest"><img src="/logo-bys.svg" alt="" aria-hidden="true" className="h-7 w-7" />Before You Send<span className="text-forest-soft">.</span></a><div className="card mt-10 p-7">
    {!authReady ? (

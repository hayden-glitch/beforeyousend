...
async function streamNDJSON(path:string,payload:Record<string,unknown>,onEvent:(e:ReviewEvent)=>void,signal?:AbortSignal,copy:StreamCopy = REVIEW_COPY){
  ...
  // 55s (conversion-cycle-2 2026-08-13, down from 75s): the hosting function's
  // ~60s hard ceiling would beat a 75s client bound with a raw 504 — the
  // controlled retry must fire FIRST so the dad gets the calm "retry —
  // nothing's lost" path instead of a dead page. The server's own ~45s
  // provider deadline usually fires first anyway; this bound is the backstop
  // for a server that never responds at all, and 55s keeps that worst-case
  // wait reasonable for a mobile dad (the ~12s stalled line in ReviewResults
  // resets the expectation long before).

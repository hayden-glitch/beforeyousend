...
async function streamNDJSON(path:string,payload:Record<string,unknown>,onEvent:(e:ReviewEvent)=>void,signal?:AbortSignal,copy:StreamCopy = REVIEW_COPY){
  ...
  // 55s: the server's own 45s provider deadline should fire first; this bound
  // is the backstop for a server that accepts the request but never returns a
  // terminal response. It lands before the hosting function's ~60s hard limit,
  // so the visitor sees a controlled retry rather than a dropped connection.

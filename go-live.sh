...
#   SAMBANOVA_API_KEY, LLM_MODEL, STRIPE_SECRET_KEY, EMAIL_API_KEY
#                   (optional) — loaded from .env and passed as runtime env vars.
...
ENV_ARGS=()
# Read deploy-time secrets from the local, gitignored .env without sourcing arbitrary code.
if [ -f .env ]; then
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in
      SAMBANOVA_API_KEY=*|LLM_MODEL=*|STRIPE_SECRET_KEY=*|EMAIL_API_KEY=*|DATABASE_URL=*|PRICE_*=*|GOOGLE_ADS_ID=*|META_PIXEL_ID=*|TIKTOK_PIXEL_ID=*|TIKTOK_ACCESS_TOKEN=*|GOOGLE_CV_REVIEW=*|GOOGLE_CV_EMAIL=*|GOOGLE_CV_ACCOUNT=*|GOOGLE_CV_CHECKOUT=*|GOOGLE_CV_SUBSCRIPTION=*)
        key="${line%%=*}"
        value="${line#*=}"
...
# Environment variables remain supported for existing DATABASE_URL deployments.
...
# Keep the stable URL pointing at the newest deployment (idempotent). The domain
# must already be attached to the project (before-you-send.vercel.app is).
STABLE_DOMAIN="${STABLE_DOMAIN:-before-you-send.vercel.app}"
if [ -n "$STABLE_DOMAIN" ]; then
  if $VERCEL alias set "$LIVE_URL" "$STABLE_DOMAIN" --token "$VERCEL_TOKEN" "${SCOPE_ARGS[@]}" >/dev/null 2>&1; then
    echo "STABLE: https://$STABLE_DOMAIN"
  else
    echo "warning: could not alias stable domain $STABLE_DOMAIN" >&2
  fi
fi
